...
class Client:
    ...
    def actually_send(s: socket.socket, file: pathlib.WindowsPath):
        ...
        progress = get_progress_bar(f'Sending {file.name}', size)
...
class Server:
    ...
    def receive(self):
        ...
        Server.actually_receive(s, client_socket, int(filesize))

    @staticmethod
    def actually_receive(s: socket.socket, cs: socket.socket, file: pathlib.WindowsPath, size: int):
        progress = get_progress_bar(f'Receiving {file.name}', size)
...
if __name__ == '__main__':
    ...
    parser.add_argument('client', required=True, const=True, type=bool, help='Start client', nargs='?')
    parser.add_argument('server', required=True, const=True, type=bool, help='Start server', nargs='?')
